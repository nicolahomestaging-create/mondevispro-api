...
def generer_pdf_devis(data: DevisRequest, numero_devis_force: Optional[str] = None) -> str:
    ...
    # CORRECTION : Calculer la TVA par taux à partir des prestations (comme dans dessiner_totaux)
    # au lieu d'utiliser data.tva_taux qui est un taux global
    # Cette logique doit être IDENTIQUE à celle de dessiner_totaux pour garantir la cohérence
    tva_par_taux_calc = {}
    for prestation in data.prestations:
        total_ligne = prestation.quantite * prestation.prix_unitaire
        if total_ligne > 0:  # Ignorer les acomptes
            # Utiliser le taux de TVA de la prestation si disponible, sinon le taux global
            taux = getattr(prestation, 'tva_taux', None) or data.tva_taux
            if taux not in tva_par_taux_calc:
                tva_par_taux_calc[taux] = 0
            # Appliquer la remise proportionnellement (comme dans dessiner_totaux)
            if remise > 0:
                ratio_remise = (total_ht_avant_acompte - remise) / total_ht_avant_acompte if total_ht_avant_acompte > 0 else 1
                total_ligne_apres_remise = total_ligne * ratio_remise
            else:
                total_ligne_apres_remise = total_ligne
            # Déduire l'acompte si présent (pas d'acompte dans les devis, mais garder pour cohérence)
            total_ligne_final = total_ligne_apres_remise
            tva_par_taux_calc[taux] += total_ligne_final * (taux / 100)
    
    # Si pas de prestations avec TVA, utiliser le calcul simple
    if not tva_par_taux_calc:
        montant_tva_total = total_ht_final * (data.tva_taux / 100)
        if data.tva_taux > 0:
            tva_par_taux_calc[data.tva_taux] = montant_tva_total
        else:
            tva_par_taux_calc[0] = 0
    else:
        # Calculer le montant total de TVA
        montant_tva_total = sum(tva_par_taux_calc.values())
    
    # Calculer le total TTC
    total_ttc = total_ht_final + montant_tva_total
    total_ht = total_ht_avant_acompte  # Pour l'affichage
    
    print(f"📊 Calcul TVA par taux - total_ht_final: {total_ht_final:.2f}, montant_tva_total: {montant_tva_total:.2f}, total_ttc: {total_ttc:.2f}")
    print(f"   tva_par_taux: {tva_par_taux_calc}")
